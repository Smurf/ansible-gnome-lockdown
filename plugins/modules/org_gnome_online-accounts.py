...
DOCUMENTATION = r'''
---
module: org_gnome_online-accounts
short_description: Manages GNOME GSettings for org.gnome.online-accounts
description:
  - >
    This module allows for the configuration of GSettings keys within the 'org.gnome.online-accounts' schema.
  - >
...
options:
  whitelisted-providers:
    description:
      ...
      - >
        A list of strings representing the providers that are allowed to be loaded (default: 'all'). This is only evaluated on startup. Deprecated: use goa.conf in the system config directory instead, with a key [providers] enable=all instead. The key is comma-separated list of the provider names.
    type: str
    default: >
             ['all']
  whitelisted-providers_locked:
    description:
      - >
        If set to true, locks the 'whitelisted-providers' key to prevent user modification.
    type: bool
    default: false
'''

EXAMPLES = r'''
- name: Configure and lock GNOME desktop settings for org.gnome.online-accounts
  org_gnome_online-accounts:
    whitelisted-providers: "['all']"
    whitelisted-providers_locked: true
'''
...
def main():
    ...
    settings_file_path = os.path.join(settings_dir, "00-ansible-org_gnome_online-accounts")
    lock_file_path = os.path.join(locks_dir, "ansible-org_gnome_online-accounts")

    schema_full_path = "/org/gnome/online-accounts/"
