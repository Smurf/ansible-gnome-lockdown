...
DOCUMENTATION = r'''
---
module: org_virt_manager_virt_manager_console
short_description: Manages GNOME GSettings for org.virt-manager.virt-manager.console
description:
  - >
    This module allows for the configuration of GSettings keys within the 'org.virt-manager.virt-manager.console' schema.
  - >
...
options:
  scaling:
    ...
    - When to scale the VM graphical console. 0 = never, 1 = only when in full screen
      mode, 2 = Always
    type: int
    default: 2
  ...
  resize-guest:
    ...
    - Automatically change guest resolution along with virt-manager window. Only works
      with spice with a vdagent set up. -1 = global default, 0 = off, 1 = on.
    type: int
    default: -1
  ...
  grab-keys_locked:
    description:
    - >
      If set to true, locks the 'grab-keys' key to prevent user modification.
    type: bool
    default: false
  auto-redirect:
    description:
    - Enable SPICE Auto USB redirection in console window
    - Whether to enable SPICE Auto USB redirection while connected to the guest console.
    type: bool
    default: true

  auto-redirect_locked:
    description:
    - >
      If set to true, locks the 'auto-redirect' key to prevent user modification.
    type: bool
    default: false
  autoconnect:
    description:
    - Autoconnect to the default VM console when the VM window is opened
    - Autoconnect to the default VM console when the VM window is opened. Users may want
      to turn this off if they prefer to use another viewer app for their VMs, and don't
      want virt-manager to interfere, but they still want to use virt-manager's details.
    type: bool
    default: true
...
EXAMPLES = r'''
- name: Configure and lock GNOME desktop settings for org.virt-manager.virt-manager.console
  org_virt_manager_virt_manager_console:
    scaling: 2
...
def main():
    ...
    settings_file_path = os.path.join(settings_dir, "00-ansible-org.virt-manager.virt-manager.console")
    lock_file_path = os.path.join(locks_dir, "ansible-org.virt-manager.virt-manager.console")

    schema_full_path = "/org/virt-manager/virt-manager/console/"
