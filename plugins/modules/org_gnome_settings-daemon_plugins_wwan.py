...
DOCUMENTATION = r'''
---
module: org_gnome_settings-daemon_plugins_wwan
short_description: Manages GNOME GSettings for org.gnome.settings-daemon.plugins.wwan
description:
  - This module allows for the configuration of GSettings keys within the 'org.gnome.settings-daemon.plugins.wwan' schema.
...
options:
  unlock-sim:
    description:
      - "Unlock sim cards"
      - "Unlock any sim cards right away."
    type: bool
    default: False
  unlock-sim_locked:
    description:
      - "If set to true, locks the 'unlock-sim' key to prevent user modification."
    type: bool
    default: false
'''

EXAMPLES = r'''
- name: Configure and lock GNOME desktop settings for org.gnome.settings-daemon.plugins.wwan
  org_gnome_settings-daemon_plugins_wwan:
    unlock-sim: False
    unlock-sim_locked: true
'''
...
def main():
    ...
    settings_file_path = os.path.join(settings_dir, "00-ansible-org_gnome_settings-daemon_plugins_wwan")
    lock_file_path = os.path.join(locks_dir, "ansible-org_gnome_settings-daemon_plugins_wwan")

    schema_full_path = "/org/gnome/settings-daemon/plugins/wwan/"
