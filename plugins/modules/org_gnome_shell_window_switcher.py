...
DOCUMENTATION = r'''
---
module: org_gnome_shell_window_switcher
short_description: Manages GNOME GSettings for org.gnome.shell.window-switcher
description:
  - >
    This module allows for the configuration of GSettings keys within the 'org.gnome.shell.window-switcher' schema.
  - >
...
options:
  app-icon-mode:
    ...
    - "Configures how the windows are shown in the switcher. Valid possibilities\n\tare\
      \ 'thumbnail-only' (shows a thumbnail of the window), 'app-icon-only'\n\t(shows\
      \ only the application icon) or 'both'."
    type: str
    default: both

  app-icon-mode_locked:
    description:
    - >
      If set to true, locks the 'app-icon-mode' key to prevent user modification.
    type: bool
    default: false
  ...
  current-workspace-only_locked:
    description:
    - >
      If set to true, locks the 'current-workspace-only' key to prevent user modification.
    type: bool
    default: false
'''

EXAMPLES = r'''
- name: Configure and lock GNOME desktop settings for org.gnome.shell.window-switcher
  org_gnome_shell_window_switcher:
    app-icon-mode: both
...
def main():
    ...
    settings_file_path = os.path.join(settings_dir, "00-ansible-org.gnome.shell.window-switcher")
    lock_file_path = os.path.join(locks_dir, "ansible-org.gnome.shell.window-switcher")

    schema_full_path = "/org/gnome/shell/window-switcher/"
