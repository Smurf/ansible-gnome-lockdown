...
DOCUMENTATION = r'''
---
module: org_virt_manager_virt_manager
short_description: Manages GNOME GSettings for org.virt-manager.virt-manager
description:
  - >
    This module allows for the configuration of GSettings keys within the 'org.virt-manager.virt-manager' schema.
  - >
...
options:
  ...
  system-tray_locked:
    description:
    - >
      If set to true, locks the 'system-tray' key to prevent user modification.
    type: bool
    default: false
  xmleditor-enabled:
    description:
    - Enable XML editing UI
    - Enable XML editing UI
    type: bool
    default: false

  xmleditor-enabled_locked:
    description:
    - >
      If set to true, locks the 'xmleditor-enabled' key to prevent user modification.
    type: bool
    default: false
  enable-libguestfs-vm-inspection:
    description:
    - Enable libguestfs VM inspection
    - Enable libguestfs VM inspection for things like OS icons, installed applications,
      etc. This only works if python libguestfs bindings are installed.
    type: bool
    default: false

  enable-libguestfs-vm-inspection_locked:
    description:
    - >
      If set to true, locks the 'enable-libguestfs-vm-inspection' key to prevent user modification.
    type: bool
    default: false
  ...
  manager-window-height_locked:
    description:
    - >
      If set to true, locks the 'manager-window-height' key to prevent user modification.
    type: bool
    default: false
  ...
  manager-window-width_locked:
    description:
    - >
      If set to true, locks the 'manager-window-width' key to prevent user modification.
    type: bool
    default: false
'''

EXAMPLES = r'''
- name: Configure and lock GNOME desktop settings for org.virt-manager.virt-manager
  org_virt_manager_virt_manager:
    system-tray: false

    xmleditor-enabled: false

    enable-libguestfs-vm-inspection: false

    manager-window-height: 0

    manager-window-width: 0
...
def main():
    ...
    settings_file_path = os.path.join(settings_dir, "00-ansible-org.virt-manager.virt-manager")
    lock_file_path = os.path.join(locks_dir, "ansible-org.virt-manager.virt-manager")

    schema_full_path = "/org/virt-manager/virt-manager/"
