...
DOCUMENTATION = r'''
---
module: org_gnome_desktop_media_handling
...
options:
  ...
  automount-open:
    ...
    - If set to true, then Nautilus will automatically open a folder when media is automounted.
      This only applies to media where no known x-content/* type was detected; for media
      where a known x-content type is detected, the user configurable action will be taken
      instead.
    type: bool
    default: true
...
- name: Configure and lock GNOME desktop settings for org.gnome.desktop.media-handling
  org_gnome_desktop_media_handling:
    automount: true
...
def main():
    ...
    settings_file_path = os.path.join(settings_dir, "00-ansible-org.gnome.desktop.media-handling")
    lock_file_path = os.path.join(locks_dir, "ansible-org.gnome.desktop.media-handling")
