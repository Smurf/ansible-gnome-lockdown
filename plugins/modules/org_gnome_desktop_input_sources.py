...
DOCUMENTATION = r'''
---
module: org_gnome_desktop_input_sources
...
options:
  ...
  sources:
    ...
    - "List of input source identifiers available. Each source is\n        specified as\
      \ a tuple of 2 strings. The first string is the\n        type and can be one of\
      \ 'xkb' or 'ibus'. For 'xkb' sources the\n        second string is 'xkb_layout+xkb_variant'\
      \ or just 'xkb_layout'\n        if a XKB variant isn't needed. For 'ibus' sources\
      \ the second\n        string is the IBus engine name. An empty list means that the\
...
- name: Configure and lock GNOME desktop settings for org.gnome.desktop.input-sources
  org_gnome_desktop_input_sources:
    current: 0
