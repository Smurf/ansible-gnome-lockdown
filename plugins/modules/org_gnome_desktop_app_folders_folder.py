#!/usr/bin/python
# -*- coding: utf-8 -*-

# Copyright (c) 2023, Jules
# All rights reserved.

from __future__ import absolute_import, division, print_function
__metaclass__ = type

DOCUMENTATION = r'''
---
module: org_gnome_desktop_app_folders_folder
short_description: Manages GNOME GSettings for org.gnome.desktop.app-folders.folder
description:
  - >
    This module allows for the configuration of GSettings keys within the 'org.gnome.desktop.app-folders.folder' schema.
  - >
    It supports setting default values and locking keys to enforce system-wide policies.
author:
  - Autogenerated by ansible-gnome-lockdown
version_added: "1.1.0"
options:
  name:
    description:
    - Folder name
    - The name of the application folder.
    type: str
    default: ''

  name_locked:
    description:
    - >
      If set to true, locks the 'name' key to prevent user modification.
    type: bool
    default: false
  translate:
    description:
    - Translate the name
    - "Whether the name key is a filename to be looked up in\n      /usr/share/desktop-directories."
    type: bool
    default: false

  translate_locked:
    description:
    - >
      If set to true, locks the 'translate' key to prevent user modification.
    type: bool
    default: false
  apps:
    description:
    - Applications
    - "The list of IDs of applications that are explicitly\n      included in this folder."
    type: str
    default: []

  apps_locked:
    description:
    - >
      If set to true, locks the 'apps' key to prevent user modification.
    type: bool
    default: false
  categories:
    description:
    - Categories
    - "A list of categories for which apps will be placed\n      into this folder by default,\
      \ in addition to the apps that are\n      listed in the apps key."
    type: str
    default: []

  categories_locked:
    description:
    - >
      If set to true, locks the 'categories' key to prevent user modification.
    type: bool
    default: false
  excluded-apps:
    description:
    - Excluded applications
    - "A list of IDs of applications that are excluded\n      from this folder. This can\
      \ be used to remove applications that\n      would otherwise be included by category."
    type: str
    default: []

  excluded-apps_locked:
    description:
    - >
      If set to true, locks the 'excluded-apps' key to prevent user modification.
    type: bool
    default: false
'''

EXAMPLES = r'''
- name: Configure and lock GNOME desktop settings for org.gnome.desktop.app-folders.folder
  org_gnome_desktop_app_folders_folder:
    name: ''

    translate: false

    apps: []

    categories: []

    excluded-apps: []

'''

RETURN = r'''
# Default return values
'''

from ansible.module_utils.basic import AnsibleModule
import os
import configparser

def main():
    """Main function for the Ansible module."""
    # We have to do gross stuff with the bools here.
    keys_spec = {
        'name': {
            'default': '',
            'type': 'str',
            'gtype':'s',
        },
        'translate': {
            'default': False,
            'type': 'bool',
            'gtype':'b',
        },
        'apps': {
            'default': [],
            'type': 'str',
            'gtype':'as',
        },
        'categories': {
            'default': [],
            'type': 'str',
            'gtype':'as',
        },
        'excluded-apps': {
            'default': [],
            'type': 'str',
            'gtype':'as',
        },
    }

    argument_spec = {}
    for key_name, spec in keys_spec.items():
        argument_spec[key_name] = spec
        argument_spec[f"{key_name}_locked"] = {'type': 'bool', 'default': False}

    module = AnsibleModule(
        argument_spec=argument_spec,
        supports_check_mode=True
    )

    changed = False

    settings_dir = "/etc/dconf/db/local.d"
    locks_dir = os.path.join(settings_dir, "locks")

    settings_file_path = os.path.join(settings_dir, "00-ansible-org.gnome.desktop.app-folders.folder")
    lock_file_path = os.path.join(locks_dir, "ansible-org.gnome.desktop.app-folders.folder")

    schema_full_path = "None"
    config_section = schema_full_path.strip('/')

    os.makedirs(settings_dir, exist_ok=True)
    os.makedirs(locks_dir, exist_ok=True)

    # Idempotency: Read current settings
    current_settings = configparser.ConfigParser()
    if os.path.exists(settings_file_path):
        current_settings.read(settings_file_path)

    # Idempotency: Read current locks from our dedicated lock file
    current_locks = []
    if os.path.exists(lock_file_path):
        with open(lock_file_path, 'r') as f:
            current_locks = [line.strip() for line in f if line.strip()]

    # Prepare settings and locks based on module parameters
    new_settings = configparser.ConfigParser()
    new_settings.add_section(config_section)
    new_locks = []

    for key_name, spec in keys_spec.items():
        param_value = module.params[key_name]
        param_locked = module.params[f"{key_name}_locked"]

        if spec['type'] == 'bool':
            setting_value = str(param_value).lower()
        elif spec['type'] == 'str':
            setting_value = f'{param_value}'
        else:
            setting_value = str(param_value)

        new_settings.set(config_section, key_name, setting_value)

        if param_locked:
            new_locks.append(f"{schema_full_path}{key_name}")

    # Check for changes
    settings_changed = False
    if not current_settings.has_section(config_section) or \
       (current_settings.has_section(config_section) and dict(current_settings.items(config_section)) != dict(new_settings.items(config_section))):
        settings_changed = True

    locks_changed = sorted(new_locks) != sorted(current_locks)

    if settings_changed or locks_changed:
        changed = True

    if changed and not module.check_mode:
        # Write settings file
        with open(settings_file_path, 'w') as f:
            new_settings.write(f, space_around_delimiters=False)

        # Manage the dedicated lock file
        if new_locks:
            with open(lock_file_path, 'w') as f:
                for lock_key in new_locks:
                    f.write(f"{lock_key}\n")
        elif os.path.exists(lock_file_path):
            os.remove(lock_file_path)

        # Update dconf database
        module.run_command(["dconf", "update"], check_rc=True)

    module.exit_json(changed=changed)

if __name__ == '__main__':
    main()
